...
// ShaderBackground â€” generated by the 21st.dev Shader Builder
// ("Waves"). Zero dependencies: one WebGL
// canvas that fills its parent. Drop it behind your content with
// <div className="relative"><ShaderBackground className="absolute inset-0" />â€¦
...
const UNIFORMS = {
  // Paleta do projeto: ink, navy, blue, cyan e magenta.
  colors: [[0.0275,0.0353,0.1176],[0.0667,0.149,0.3373],[0.0549,0.3294,0.7412],[0,0.8118,1],[0.851,0.1373,0.6471],[0.851,0.1373,0.6471],[0.851,0.1373,0.6471],[0.851,0.1373,0.6471]] as [number, number, number][],
  colorCount: 5,
  scale: 2.000,
  intensity: 0.460,
  paramA: 0.470,
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const effectiveTimeScale = reduceMotion || saveData ? 0 : UNIFORMS.timeScale
    ...
    const timeAnimated = Math.abs(effectiveTimeScale) > 0.0001
    ...
    const resizeCanvas = () => {
      ...
      const pixelScale = Math.min(
        1,
        Math.sqrt(1_500_000 / Math.max(1, rawWidth * rawHeight)),
      )
    ...
    function render(now: number) {
      ...
      gl.uniform4f(
        ...
        width,
        height,
        ((now - start) / 1000) * effectiveTimeScale,
        UNIFORMS.colorCount,
      )
      gl.uniform4f(
        uni.space,
